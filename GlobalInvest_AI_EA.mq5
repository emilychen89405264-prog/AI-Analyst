//+------------------------------------------------------------------+
//|  GlobalInvest AI EA v2.0                                         |
//|  策略：多時框EMA趨勢 + ATR止損 + 連虧保護 + R:R≥2               |
//|  適用：XAUUSD / GBPUSD / EURUSD                                  |
//+------------------------------------------------------------------+
...
input double   InpTrailStart    = 0.25;      // 達到 TP 25% 時啟動移動止損
input double   InpBreakevenAt   = 0.15;      // 達到 TP 15% 時移至保本
...
int OnInit()
{
   ...
   if(hEmaFast_HTF == INVALID_HANDLE || hEmaSlow_HTF == INVALID_HANDLE ||
      ...
      hAtr_LTF     == INVALID_HANDLE)
   {
      Print("❌ [EA Init] 指標建立失敗，請確認品種正確");
      return INIT_FAILED;
   }
   
   double startLot = (gSymbol == "XAUUSD" || gSymbol == "XAUUSD.") ? InpLotGold :
                     (gSymbol == "GBPUSD" || gSymbol == "GBPUSD.") ? InpLotGBPUSD : InpLotDefault;
   Print("✅ [GlobalInvest AI v2] 啟動成功 | 品種:", gSymbol,
         " | Magic:", gMagicNumber,
         " | FixedLot:", startLot,
         " | R:R≥", InpRR_Ratio);
...
void ManageOpenPositions()
{
   for(int i = PositionsTotal()-1; i >= 0; i--)
   {
      ...
      // 保本：達到 TP 的 InpBreakevenAt（預設15%）時移至保本
      if(progress >= InpBreakevenAt && InpUseTrailing)
      {
         double bePrice = entryPrice + (tp - entryPrice) * 0.02; // 保本+小盈利緩衝
         if(posInfo.PositionType() == POSITION_TYPE_BUY && bePrice > sl + Point())
            newSL = bePrice;
         else if(posInfo.PositionType() == POSITION_TYPE_SELL && bePrice < sl - Point())
            newSL = bePrice;
      }
      
      // 移動止損：達到 TP 的 InpTrailStart（預設25%）後，SL 跟著走
...
double CalcLotSize(double slDistance)
{
   ...
   // 依品種選擇固定手數
   if(gSymbol == "XAUUSD" || gSymbol == "XAUUSD.")
      lot = InpLotGold;
   else if(gSymbol == "GBPUSD" || gSymbol == "GBPUSD.")
      lot = InpLotGBPUSD;
   else
      lot = InpLotDefault;
