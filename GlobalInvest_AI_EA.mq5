...
input group "=== XAUUSD 專屬設定 ==="
input double   InpMinConfGold    = 85.0;      // XAUUSD 最低進場分數（比一般更嚴格）
input double   InpADXMinGold     = 25.0;      // XAUUSD 最低 ADX（需更強趨勢）
input bool     InpUseRSI         = true;      // 啟用 RSI 方向確認
...
int OnInit()
{
   ...
   hRsi_LTF     = iRSI(gSymbol, InpLTF,  InpRSIPeriod, PRICE_CLOSE);
...
void OnTick()
{
   ...
   // === ADX 震盪過濾：4H ADX 低於門值就是震盪市場，不進場 ===
   double adxVal = 0;
   if(InpUseADX)
   {
      double adxBuf[2];
      if(CopyBuffer(hAdx_HTF, 0, 0, 2, adxBuf) >= 2) adxVal = adxBuf[1];
      bool isGoldAdx = (StringFind(gSymbol, "XAU") >= 0);
      double adxThreshold = isGoldAdx ? InpADXMinGold : InpADXMin; // 黃金用更高門值
      if(adxVal < adxThreshold)
      {
         Print("⏸️ [ADX震盪] 4H ADX=", DoubleToString(adxVal,1), " < ", adxThreshold, "，震盪市場不進場");
         return;
      }
   }
   ...
   // === XAUUSD 特殊規則：做多做空都需三時框完全同步 ===
   bool isGold = (gSymbol == "XAUUSD" || gSymbol == "XAUUSD.");
   if(isGold)
   {
      if(trendLTF != direction)
      {
         Print("🔒 [XAUUSD] H1 未確認方向，跳過（需 D1+H4+H1 三時框同步）");
         return;
      }
      if(direction == -1) score -= 10; // 黃金做空額外扮10分
   }
   ...
   {
      double rsiBuf[2];
      if(CopyBuffer(hRsi_LTF, 0, 0, 2, rsiBuf) >= 2) rsiVal = rsiBuf[1];
      if(direction == 1)
      {
         if(rsiVal > 55 && rsiVal <= 70)  score += 10; // 健康多頭區
         else if(rsiVal > 70)             score -= 15; // 超買，不追高
      }
      else
      {
         if(rsiVal < 45 && rsiVal >= 30)  score += 10; // 健康空頭區
         else if(rsiVal < 30)             score -= 15; // 超賣，不追低
      }
   }
   
   // === 動量K棒確認：實體要夠大，排除十字星 ===
   MqlRates kBars[2];
   if(CopyRates(gSymbol, InpLTF, 0, 2, kBars) >= 2)
   {
      double bodySize = MathAbs(kBars[1].close - kBars[1].open);
      bool   bodyDir  = (direction == 1) ? (kBars[1].close > kBars[1].open)
                                        : (kBars[1].close < kBars[1].open);
      if(bodySize >= atr[1] * 0.4 && bodyDir)  score += 10; // 強方向K棒
      else if(bodySize < atr[1] * 0.15)         score -= 10; // Doji，方向不明
   }
   
   Print("📊 [評分] ", gSymbol, " | D1:", trendD1, " 4H:", trendHTF, " 1H:", trendLTF,
         " | ADX:", DoubleToString(adxVal,1), " RSI:", DoubleToString(rsiVal,1),
         " | CrossBonus:", freshCrossLTF ? 15 : 0, " | Pullback:", pullbackScore,
         " | 總分:", score);
   
   // === 門檻判斷（黃金用更高專屬門檻）===
   double threshold = isGold ? InpMinConfGold : InpMinConfidence;
   if(score < threshold)
   {
      Print("⏸️ [HOLD] 評分 ", score, " 低於門檻 ", threshold, "，跳過");
      return;
   }
   
   // === UJ 限制只做多 ===
   if(direction == -1 && (StringFind(gSymbol, "USDJPY") >= 0 || StringFind(gSymbol, "JPY") >= 0))
   {
      Print("🔒 [USDJPY] 限制只做多，忽略做空訊號");
      return;
   }
